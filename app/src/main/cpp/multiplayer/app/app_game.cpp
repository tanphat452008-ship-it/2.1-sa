...
void Render2dStuff()
{
    ...
    if (CHUD::bIsShow) {
        if (CGame::IsToggledHUDElement(HUD_ELEMENT_HUD)) {
            // NOTE: The offsets 0x51CFF0 and 0x55CC18 might be incorrect for this version of libGTASA.so
            // and are causing crashes (calling CTempColModels::Initialise instead).
            // We will rely on CTouchInterface::DrawAll for now which draws the mobile HUD.

            // ((void (*)()) (g_libGTASA + (VER_x32 ? 0x00437B0C + 1 : 0x51CFF0)))(); // CHud::DrawRadar
            // ((void (*)()) (g_libGTASA + (VER_x32 ? 0x00470928 + 1 : 0x55CC18)))(); // CHud::Draw
        }

        // Draw All Touch Widgets (including native health bars, weapon icons, etc.)
        if (CHUD::bIsShow && !CKeyBoard::m_bEnable && CTouchInterface::m_pWidgets) {
            auto radar = CTouchInterface::m_pWidgets[0xA1]; // WIDGET_RADAR
            if (radar) {
            ...
            ((void (*)(bool)) (g_libGTASA + (VER_x32 ? 0x002B0BD8 + 1 : 0x36FB00)))(
                    false); // CTouchInterface::DrawAll
        }

        // Render TextDraws and GameText (Important for Login/Server Info)
        CHook::CallFunction<void>(g_libGTASA + (VER_x32 ? 0x001C0750 + 1 : 0x252CE4),
                                  1); // CTextDraw::Render
        ((void (*)(bool)) (g_libGTASA + (VER_x32 ? 0x0054BDD4 + 1 : 0x66B678)))(
                true); // CMessages::Display
        ((void (*)(bool)) (g_libGTASA + (VER_x32 ? 0x005A9120 + 1 : 0x6CCEA0)))(true);
    }
}