...
uint16_t *szGameTextMessage = nullptr;
...
void CGame::Init()
{
    ...
    memset(&bUsedPlayerSlots[0], 0, sizeof(bUsedPlayerSlots));
}
...
void CGame::RemovePlayer(CPedSamp* pPlayer)
{
    if(pPlayer)
    {
        if(pPlayer->m_bytePlayerNumber < PLAYER_PED_SLOTS) {
...
uintptr CGame::CreatePickup(int iModel, int iType, CVector* pos, uint32* unk)
{
    ...
    if(!dwModelArray || dwModelArray[iModel] == nullptr)
        iModel = 18631; // Icon chấm hỏi mặc định khi thiếu model
...
{
    if(pos) memcpy(&m_vecCheckpointPos, pos, sizeof(CVector));
    if(extent) memcpy(&m_vecCheckpointExtent, extent, sizeof(CVector));
}

// 0.3.7
void CGame::SetRaceCheckpointInformation(uint8_t byteType, CVector *pos, CVector *next, float fSize)
{
    if(pos) memcpy(&m_vecRaceCheckpointPos, pos, sizeof(CVector));
    if(next) memcpy(&m_vecRaceCheckpointNext, next, sizeof(CVector));
...
void CGame::RaceCheckpointPicked() {
    if(!m_bRaceCheckpointsEnabled || !pNetGame || !pNetGame->GetRakClient())
        return;
...
bool CGame::InitialiseRenderWare() {
    ...
    // FIX CRASH: Kiểm tra an toàn khởi tạo Camera & Raster RenderWare
    ...
    if (!camera->frameBuffer || !camera->zBuffer || !camera->object.object.parent) {
        CameraDestroy(camera);
        return false;
    }
    ...
    pGUI = new CGUI(ImVec2(RsGlobal->maximumWidth, RsGlobal->maximumHeight), font_path.c_str());
    if (pGUI) {
        pGUI->initialize();
        pGUI->performLayout();
    }
...
void CGame::Process() {
    ...
    // Gọi hàm Game với địa chỉ đã kiểm tra an toàn
    auto CallGameFunc = [](uintptr_t addr) {
        if(addr) ((void(*)())addr)();
    };

    CallGameFunc(g_libGTASA + (VER_x32 ? 0x003F8B50 + 1 : 0x4DB464)); // CPad::UpdatePads()
    CallGameFunc(g_libGTASA + (VER_x32 ? 0x002B03F8 + 1 : 0x36F374)); // CTouchInterface::Clear()
    CallGameFunc(g_libGTASA + (VER_x32 ? 0x0028C178 + 1 : 0x3467BC)); // CHID::Update()

    CStreaming::Update();

    if (!(CTimer::m_CodePause << 0x18))
    {
        uintptr_t gMobileMenuPtr = (g_libGTASA + (VER_x32 ? 0x006E0074 : 0x8BE780));
        if (gMobileMenuPtr && *(uintptr_t*)gMobileMenuPtr) {
            auto gMobileMenu = (uintptr_t *)*(uintptr_t*)gMobileMenuPtr;
            uintptr_t updateFunc = (g_libGTASA + (VER_x32 ? 0x0029A730 + 1 : 0x356A7C));
            if (updateFunc) ((void(*)(uintptr_t*))updateFunc)(gMobileMenu);
        }
    }
    ...
    *(int32_t*)(g_libGTASA + (VER_x32 ? 0x00A7D22C: 0xD217F8)) = 0; // CWindModifiers::Number
    ...
    if (!CTimer::m_CodePause && !CTimer::m_UserPause)
    {
        ...
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005C89F8 + 1 : 0x6ECF00)); // CSprite2d::InitPerFrame();
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005A8A74 + 1 : 0x6CC898)); // CFont::InitPerFrame()
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005CC2E8 + 1 : 0x6F0BD8)); // CWeather::Update()
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x0032AED8 + 1 : 0x3F3AD8)); // CTheScripts::Process()

        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005BE838 + 1 : 0x6E2F08)); // CSkidmarks::Update();
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005AB4C8 + 1 : 0x6D032C)); // CGlass::Update()

        uintptr_t gFireManagerPtr = (g_libGTASA + (VER_x32 ? 0x00958800 : 0xBC12D8));
        if (gFireManagerPtr && *(uintptr_t*)gFireManagerPtr) {
            auto gFireManager = (uintptr_t *)*(uintptr_t*)gFireManagerPtr;
            uintptr_t updateFire = (g_libGTASA + (VER_x32 ? 0x003F1628 + 1 : 0x4D361C));
            if (updateFire) ((void (*)(uintptr_t *))updateFire)(gFireManager);
        }

        uintptr_t popUpdate = (g_libGTASA + (VER_x32 ? 0x004CC380 + 1 : 0x5CB5E0));
        if (popUpdate) ((void(*)(bool))popUpdate)(false); // CPopulation::Update

        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005DB8E8 + 1 : 0x700AF4)); // CWeapon::UpdateWeapons()
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005A6720 + 1 : 0x6CA130)); // CMovingThings::Update();
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005CBB20 + 1 : 0x6F04CC)); // CWaterCannons::Update()
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x00427744 + 1 : 0x50BE40)); // CWorld::Process()

        if (!CTimer::bSkipProcessThisFrame)
        {
            CPickups::Update();
            CallGameFunc(g_libGTASA + (VER_x32 ? 0x0059CFC0 + 1 : 0x6C13F0)); // CBirds::Update()
            CallGameFunc(g_libGTASA + (VER_x32 ? 0x005C03E4 + 1 : 0x6E4A7C)); // CSpecialFX::Update()
        }

        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005B28D8 + 1 : 0x6D6E34)); // CPostEffects::Update()
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x0041EF78 + 1 : 0x502ADC)); // CTimeCycle::Update()

        uintptr_t camProcess = (g_libGTASA + (VER_x32 ? 0x003DC7D0 + 1 : 0x4BAB78));
        if (camProcess) ((void (*)(CCamera*))camProcess)(&CCamera::Get()); // CCamera::Process()

        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005A3E40 + 1 : 0x6C75E4)); // CCoronas::DoSunAndMoon()
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005A22C8 + 1 : 0x6C5BE0)); // CCoronas::Update()
        CallGameFunc(g_libGTASA + (VER_x32 ? 0x005BD370 + 1 : 0x6E1BC4)); // CShadows::UpdatePermanentShadows()

        CCustomBuildingRenderer::Update();

        auto temp = CCamera::Get().m_pRwCamera;
        if (temp) {
            uintptr_t g_fx_ptr = (g_libGTASA + (VER_x32 ? 0x00820520 : 0xA062A8));
            if (g_fx_ptr && *(uintptr_t*)g_fx_ptr) {
                auto g_fx = *(uintptr_t *)g_fx_ptr;
                uintptr_t fxUpdate = (g_libGTASA + (VER_x32 ? 0x00363DE0 + 1 : 0x433F48));
                if (fxUpdate) ((void (*)(uintptr_t*, RwCamera*, float ))fxUpdate)(&g_fx, temp, CTimer::ms_fTimeStep / 50.0f);
            }
        }

        uintptr_t g_breakMan_ptr = (g_libGTASA + (VER_x32 ? 0x0099DD14 : 0xC31CF0));
        if (g_breakMan_ptr && *(uintptr_t*)g_breakMan_ptr) {
            auto g_breakMan = (uintptr_t *)*(uintptr_t*)g_breakMan_ptr;
            uintptr_t breakUpdate = (g_libGTASA + (VER_x32 ? 0x0045267C + 1 : 0x53AFDC));
            if (breakUpdate) ((void (*)(uintptr_t*, float ))breakUpdate)(g_breakMan, CTimer::ms_fTimeStep);
        }

        CallGameFunc(g_libGTASA + (VER_x32 ? 0x00596540 + 1 : 0x6BB3A8)); // CWaterLevel::PreRenderWater()
    }
...
bool CGame::InitialiseEssentialsAfterRW() {
    uintptr_t TheTextAddr = (g_libGTASA + (VER_x32 ? 0xA00768 : 0xCA0888));
    if (TheTextAddr && *(uintptr_t*)TheTextAddr) {
        auto TheText = (uintptr_t *)*(uintptr_t*)TheTextAddr;
