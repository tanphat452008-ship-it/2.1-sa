...
void RequestClass(RPCParameters *rpcParams)
{
    ...
    if (byteRequestOutcome) {
        ...
        // Chuẩn SA-MP: Sau khi nhận thông tin Class, nếu muốn auto-spawn
        // thì gửi RequestSpawn ngay lập tức
...
void RequestSpawn(RPCParameters *rpcParams)
{
    ...
    if (byteRequestOutcome) {
        // Fix chuẩn SA-MP: Chỉ thực hiện Spawn khi Server xác nhận Outcome = 1
        CLocalPlayer::Spawn();
    }
...
void ProcessIncommingEvent(BYTE bytePlayerID, int iEventType, uint32_t dwParam1, uint32_t dwParam2, uint32_t dwParam3)
{
	...
	/*case EVENT_ENTEREXIT_MODSHOP:
		...
		pRemote = CPlayerPool::GetAt(bytePlayerID);
		if (pRemote && pVehicle) 
		{
...
void WorldPlayerDeath(RPCParameters* rpcParams)
{
    ...
    pRemotePlayer =  CPlayerPool::GetAt(playerId);
    if (pRemotePlayer) {
