...
void CNetGame::Process() {
   ...
   // CSnow::Process();
    ...
    if (GetGameState() == eNetworkState::CONNECTED || GetGameState() == eNetworkState::AWAIT_JOIN) {
        // pool process
...
void CNetGame::Packet_CustomRPC(Packet *p) {
    ...
    switch (rpcID) {
        ...
        case RPC_SPAWN_AT: {
            ...
            CPedSamp* ped = CLocalPlayer::GetPlayerPed();
            if (ped && ped->m_pPed)
            {
                ped->m_pPed->Teleport(pos, false);
                ped->ForceTargetRotation(rotation);
            }
