    ...
    if(m_bIsActive && m_pPlayerPed) {
        ...
        if (m_bIsSpectating  && !m_bIsActive) {
            ...
            if (m_bSpawnDialogShowed)
            {
                ...
                if(pGUI) pGUI->spawn()->setVisible(false);
            }
        }
...
void CLocalPlayer::ProcessClassSelection()
{
    if (!m_bSpawnDialogShowed)
    {
        // pGUI->spawn()->setVisible(true); // Disable UI
...
bool CLocalPlayer::Spawn()
{
    ...
    Log("CLocalPlayer::Spawn forced");

    // Tắt các UI không cần thiết
    if(pGUI) {
        pGUI->spawn()->setVisible(false);
    }
    m_bSpawnDialogShowed = false;
    ...
    // Fix Camera: Thoát chế độ Spectate và đặt camera vào người chơi
    m_bIsSpectating = false;
    m_byteSpectateType = SPECTATE_TYPE_NONE;
    ...
    if (m_pPlayerPed) {
        ...
        // Gán camera trực tiếp vào thực thể nhân vật
        CCamera::Get().pTargetEntity = m_pPlayerPed->m_pPed;
        CCamera::Get().TakeControl(m_pPlayerPed->m_pPed, eCamMode::MODE_FOLLOWPED, eSwitchType::JUMPCUT, 1);
    }

    CHUD::toggleAll(true, true);

    CHook::WriteMemory(g_libGTASA + (VER_x32 ? 0x36EA2C : 0x4A6B68), (uintptr_t)"\x70\x47", 2);

    CGame::RefreshStreamingAt(m_SpawnInfo.vecPos.x, m_SpawnInfo.vecPos.y);
    CColStore::RequestCollision(&m_SpawnInfo.vecPos, CGame::currArea);

    CVector pos = m_SpawnInfo.vecPos;
    pos.z += 1.0f;

    m_pPlayerPed->RestartIfWastedAt(&pos, m_SpawnInfo.fRotation);
    ...
    // Give weapons
    for(int i = 0; i < 3; i++) {
        if (m_SpawnInfo.iSpawnWeapons[i] != -1 && m_SpawnInfo.iSpawnWeaponsAmmo[i] > 0) {
            m_pPlayerPed->m_pPed->GiveWeapon(m_SpawnInfo.iSpawnWeapons[i], m_SpawnInfo.iSpawnWeaponsAmmo[i]);
        }
    }

    // Gửi tín hiệu đã spawn xong về server (ID_SPAWN)
