...
void CRemotePlayer::Process() {
	...
	else if (GetState() == PLAYER_STATE_DRIVER && m_pPlayerPed->m_pPed->IsInVehicle()) {
	...
	else if (GetState() == PLAYER_STATE_PASSENGER && m_pPlayerPed->m_pPed->IsInVehicle())
	{
...
void CRemotePlayer::UpdateOnFootPositionAndSpeed(CVector *vecPos, CVector *vecMove)
{
	...
	m_pPlayerPed->m_pPed->SetVelocity(*vecMove);
}
...
void CRemotePlayer::StoreOnFootFullSyncData(ONFOOT_SYNC_DATA *pofSync)
{
	...
	m_fCurrentHealth = (float)pofSync->byteHealth;
	m_fCurrentArmor = (float)pofSync->byteArmour;
...
void CRemotePlayer::StoreInCarFullSyncData(INCAR_SYNC_DATA *picSync)
{
	...
	m_byteSeatID = 0;
	m_fCurrentHealth = (float)picSync->bytePlayerHealth;
	m_fCurrentArmor = (float)picSync->bytePlayerArmour;
