...
class HudManager : Chat() {
    ...
    fun initServerLogo() {
        activity.runOnUiThread {
            ...
            val serverColorRaw = Storage.getProperty(StorageElements.SERVER_COLOR, activity)
            ...
            val safeColorString = when {
                serverColorRaw.isNullOrBlank() -> "#FFFFFFFF"       // дефолт белый
                serverColorRaw.startsWith("#") -> serverColorRaw
                else -> "#$serverColorRaw"
            }

            val mainColor = try {
                Color.parseColor(safeColorString)
            } catch (e: Exception) {
                Color.WHITE
            }
            ...
            // === БЕЗОПАСНОЕ ИМЯ СЕРВЕРА ===
            text.text = if (!serverName.isNullOrBlank()) {
                "$serverName "
            } else {
                "SERVER"
            }
            ...
            if (!serverMulti.isNullOrBlank()) {
                ...
                multiText.visibility = View.GONE
            }
        }
    }
